...
export class DepartmentConfigService {
    ...
    create(departmentConfig: any): Observable<EntityResponseType> {
        const copy = this.convertDateFromClient(departmentConfig);
        return this.http.post<RestDepartmentConfig>(this.resourceUrl, copy, { observe: 'response' }).pipe(map((res) => this.convertResponseFromServer(res)));
    }
    ...
    find(id: string): Observable<EntityResponseType> {
        return this.http.get<RestDepartmentConfig>(`${this.resourceUrl}/${id}`, { observe: 'response' }).pipe(map((res) => this.convertResponseFromServer(res)));
    }
    ...
    query(req?: any): Observable<EntityArrayResponseType> {
        ...
        return this.http.get<RestDepartmentConfig[]>(this.resourceUrl, { params: options, observe: 'response' }).pipe(map((res) => this.convertResponseArrayFromServer(res)));
    }

    fetchAcademicYears(deptId: number): Observable<any> {
        const endpoint = this.resourceUrl + `/academic-years/${deptId}`;
    ...
    addDepartmentConfigToCollectionIfMissing<Type extends Pick<IDepartmentConfig, 'id'>>(departmentConfigCollection: Type[], ...departmentConfigsToCheck: (Type | null | undefined)[]): Type[] {
        const departmentConfigs: Type[] = departmentConfigsToCheck.filter(isPresent);
        if (departmentConfigs.length > 0) {
            const departmentConfigCollectionIdentifiers = departmentConfigCollection.map((departmentConfigItem) => this.getDepartmentConfigIdentifier(departmentConfigItem));
    ...
    protected convertDateFromClient<T extends IDepartmentConfig | NewDepartmentConfig | PartialUpdateDepartmentConfig>(departmentConfig: T): RestOf<T> {
        return {
            ...departmentConfig,
