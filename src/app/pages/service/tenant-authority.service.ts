...
export class TenantAuthorityService {
    ...
    protected resourceUrl = environment.ServerUrl + environment.UAA_BASE_URL + this.applicationConfigService.getEndpointFor('authorities');
    ...
    addTenantAuthorityToCollectionIfMissing<Type extends Pick<ITenantAuthority, 'name'>>(tenantAuthorityCollection: Type[], ...tenantAuthoritiesToCheck: (Type | null | undefined)[]): Type[] {
        const tenantAuthorities: Type[] = tenantAuthoritiesToCheck.filter(isPresent);
        if (tenantAuthorities.length > 0) {
            const tenantAuthorityCollectionIdentifiers = tenantAuthorityCollection.map((tenantAuthorityItem) => this.getTenantAuthorityIdentifier(tenantAuthorityItem));
