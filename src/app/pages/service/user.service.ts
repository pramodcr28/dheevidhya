...
type RestOf<T extends ITenantUser | NewTenantUser> = Omit<T, 'createdDate' | 'resetDate' | 'lastModifiedDate'> & {
...
export class UserService {
  ...
  create(tenantUser: NewTenantUser): Observable<EntityResponseType> {
    ...
    return this.http
      .post<RestTenantUser>(this.userResourceUrl, copy, { observe: 'response' })
  ...
  update(tenantUser: ITenantUser): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(tenantUser);
    return this.http
      .put<RestTenantUser>(`${this.userResourceUrl}/${this.getTenantUserIdentifier(tenantUser)}`, copy, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
  ...
  partialUpdate(tenantUser: PartialUpdateTenantUser): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(tenantUser);
    return this.http
      .patch<RestTenantUser>(`${this.userResourceUrl}/${this.getTenantUserIdentifier(tenantUser)}`, copy, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http
      .get<RestTenantUser>(`${this.userResourceUrl}/${id}`, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http
      .get<RestTenantUser[]>(this.userResourceUrl, { params: options, observe: 'response' })
      .pipe(map(res => this.convertResponseArrayFromServer(res)));
  }

  getAuthorities(): Observable<HttpResponse<ITenantAuthority[]>> {
    const options = createRequestOption(null);
    return this.http.get<ITenantAuthority[]>(this.authorityResourceUrl, { params: options, observe: 'response' });
  }
  ...
  addTenantUserToCollectionIfMissing<Type extends Pick<ITenantUser, 'id'>>(
    tenantUserCollection: Type[],
    ...tenantUsersToCheck: (Type | null | undefined)[]
  ): Type[] {
  ...
  protected convertDateFromClient<T extends ITenantUser | NewTenantUser | PartialUpdateTenantUser>(tenantUser: T): RestOf<T> {
    return {
      ...tenantUser,
      createdDate: tenantUser.createdDate?.toJSON() ?? null,
      resetDate: tenantUser.resetDate?.toJSON() ?? null,
      lastModifiedDate: tenantUser.lastModifiedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTenantUser: RestTenantUser): ITenantUser {
    return {
      ...restTenantUser,
      createdDate: restTenantUser.createdDate ? dayjs(restTenantUser.createdDate) : undefined,
      resetDate: restTenantUser.resetDate ? dayjs(restTenantUser.resetDate) : undefined,
      lastModifiedDate: restTenantUser.lastModifiedDate ? dayjs(restTenantUser.lastModifiedDate) : undefined,
    };
  }
