...
import { AttendanceRequest, StudentAttendanceReportResponse } from '../models/attendence.model';
...
export class StudentAttendenceServiceService {
    ...
    getReports<T>(page: number = 0, size: number = 10, sortBy: string = 'id', sortDirection: string = 'ASC', filters: any = {}): Observable<any> {
        const searchRequest = {
            page: page,
            size: size,
            sortBy: sortBy,
    ...
    getStudentReport(params: any) {
        return this.http.get<StudentAttendanceReportResponse>(`${this.resourceUrl}/my-report`, { params });
    }
}
