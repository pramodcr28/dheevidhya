...
export class ProfileConfigService {
  ...
  protected resourceUrl = this.applicationConfigService.getEndpointFor(environment.ServerUrl + environment.ADMIN_BASE_URL +'api/profile-configs');
  ...
  addProfileConfigToCollectionIfMissing<Type extends Pick<IProfileConfig, 'id'>>(
    profileConfigCollection: Type[],
    ...profileConfigsToCheck: (Type | null | undefined)[]
  ): Type[] {
    const profileConfigs: Type[] = profileConfigsToCheck.filter(isPresent);
    if (profileConfigs.length > 0) {
      const profileConfigCollectionIdentifiers = profileConfigCollection.map(profileConfigItem =>
        this.getProfileConfigIdentifier(profileConfigItem),
      );
